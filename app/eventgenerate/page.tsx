...
import DateCalendar, { formatDisplay } from './DateCalendar'

type HostType = 'business' | 'community'
type EventType = 'ticket' | 'free'

export default function EventGeneratePage() {
  ...
  const handleSubmit = () => {
    ...
    router.push(`/eventmaster?${params.toString()}`)
  }
      ...
      <div className="max-w-xl mx-auto px-6 pt-12 flex flex-col gap-10">
        ...
        <Reveal visible={revealed(4) && !!isQ4Complete}>
          <button
            ...
            className="w-full py-4 rounded-2xl font-semibold text-base transition-opacity hover:opacity-85"
            style={{ backgroundColor: '#D94F35', color: '#F5F0E8' }}
          >
            이벤트 만들기 →
          </button>
